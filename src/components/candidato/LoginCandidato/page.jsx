...
export default function LoginCandidato() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Verify that the role is job_seeker (candidato) - only redirect if role matches
      if (tokenData.rol !== 'job_seeker') {
        throw new Error("Este usuario no es un candidato")
      }

      // Only proceed with redirect if role is job_seeker
      if (tokenData.rol === 'job_seeker') {
        ...
        window.location.href = `/candidato/perfil/${tokenData.user_id}`
      }
    } catch (err) {
